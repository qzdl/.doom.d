...
: t
** windows: min-width
- window width minimum for 90. -> why not try out a change

so far, kind of annoying how it blocks splits, maybe some 'warn less than LIMIT -> resize?'
would be better
#+begin_src emacs-lisp
...
#+RESULTS:
: t
*** annoying results to echo always
**** at point annotated (CIDER)
#+begin_src emacs-lisp
...
*** HACK on getting buffer info (file+line) for links
:PROPERTIES:
:ID:       43725885-d44a-4972-94de-1a063464fb21
:END:
#+begin_src emacs-lisp
...
#+RESULTS:
: qz/yank-buffer-path-and-line-num
*** org-map-entries
relevant for
- [[id:eb5bf554-0804-4487-be65-8ec1fac85d4f][data sync]] (org-jira)
- index of headlines in buffer(s)/file(s)

if headline or tag matches CONCEPT

entry backend
- flat links
  * =concept1=: link1, link2, ..., linkN
  * =concept2=: link1, link2, ..., linkN
...
[[~/dotfiles/.doom.d/config.org::*highest identity spec available]]
link reference get (highest identity spec available)
- orgid, file+headline
- precommit build task for [[id:eb5bf554-0804-4487-be65-8ec1fac85d4f][data sync]]

#+begin_src emacs-lisp :tangle no :results drawer
(defun qz/headline-concepts (concepts)
  (apply
   'append
   (remove
    nil
    (org-map-entries ;; where the unit is entries. could generalize for any such LINE
     (lambda ()
       (cl-destructuring-bind
           (level _ kword priority headline tags) (org-heading-components)
         (remove nil
                 (cl-loop for concept in concepts
                          collect (progn
                                    ;;(message "%s:%s:%s" (string-match (symbol-name concept) headline) concept headline)
                                    (when-let ((hl (string-match (symbol-name concept) headline)))
                                      (list concept
                                            (concat "[[" (f-this-file) "::*" (org-link-display-format headline) "]"
                                                    "[" (org-link-display-format headline) "]]")
                                            (locate-current-line-number))))))))))))


(defun qz/entry-index (concepts &optional buffer)
  (let ((buffer (or buffer (current-buffer))))


    ))

(defun qz/universe-entry-index (concepts &optional files)
  (let ((files (or files (org-roam-list-files))))
    ))

;; (let* ((concepts '(headline debug))
...
;;(qz/pprint (qz/headline-concepts '(headline test debug roam)))
#+end_src

#+RESULTS:
:results:

((roam "[[/home/samuel/dotfiles/.doom.d/config.org::*roam insert todo on {s-i}][roam insert todo on {s-i}]]"
       194)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*org-mode][org-mode]]"
       1971)
 (headline "[[/home/samuel/dotfiles/.doom.d/config.org::*recurring events headlines][recurring events headlines]]"
           2926)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*org-roam][org-roam]]"
       3009)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*roam counts][roam counts]]"
       3234)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*roam capture templates][roam capture templates]]"
       3250)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*point->roam-id; get id from node at point][point->roam-id; get id from node at point]]"
       3308)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*title->roam-id][title->roam-id]]"
       3324)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*custom roam-insert][custom roam-insert]]"
       3370)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*org-roam-immediate-node-insert][org-roam-immediate-node-insert]]"
       3505)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*org-roam-node-insert][org-roam-node-insert]]"
       3517)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*today's date reference as roam links][today's date reference as roam links]]"
       3638)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*data-driven automatic actions in org roam][data-driven automatic actions in org roam]]"
       3729)
 (headline "[[/home/samuel/dotfiles/.doom.d/config.org::*=get-headline-path= headline parsing function][=get-headline-path= headline parsing function]]"
           4118)
 (headline "[[/home/samuel/dotfiles/.doom.d/config.org::*query slice of headlines, containing REGEXP, build path with headline parsing function][query slice of headlines, containing REGEXP, build path with headline parsing function]]"
           4174)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*org-roam-server][org-roam-server]]"
       4176)
 (roam "[[/home/samuel/dotfiles/.doom.d/config.org::*can we please AUTO/org-roam create a missing note?][can we please AUTO/org-roam create a missing note?]]"
       4338))
:end:
...
((headline "[[~/dotfiles/.doom.d/config.org::*recurring events headlines]]" 2902)
 (headline "[[=get-headline-path= headline parsing function]]" 4094)
 (headline "[[query slice of headlines, containing REGEXP, build path with headline parsing function]]"
           4150))
:end:


*** org-share: ol-links :module:
:PROPERTIES:
:ID:       d48a91f6-d7a7-4516-a761-f270e34cd279
:END:
:: modules/org-share
[[id:6baf4bff-d5fe-4ce0-a130-0464e49de67d][org-mode]] + [[id:edd82bc1-eb33-4170-a953-051ec2c50fae][Information and the Universe]] mechanisms


**** TODO extrapolate to other link types (http, file, etc)
- pre-store: identity hierarchy
  - which identity backend for a link?
  - is this a question for storage as well as export?
    + yes, because files/views are mutable

***** identity ontology
- ipfs: SHA of file
- ipfs: SHA of (file + datetime) -> uses in adhoc
- repo+commit SHA
- func: latest file -> e.g. most roam links

what are we prioritising?
- strength of identity? -> hash -> IPFS
- strength of context? -> repo+SHA -> (constraint: filesize)
- strength of relevance? -> head of =version-view= / ipld / mutable page

to establish which identity is most suitable, address the reason for making the link
- a highlight / region -> content-based -> identity with respect to content
- denoting =oneof= -> ? (reference/link/edge based) -> identity with respect to context

***** privacy: gpg password / key / etc

****** declarative key spec :spec:
(:keys ((key-name . file-list) ...)
 :passwords ((password-name . file-list ) ...))

(key-name . public-key) -> gpg / other crypt backend
(password-name . authinfo)


so we can produce a given file-list with a set of predicates, like =private-p=
auto-roam-function

****** good to have a prompt workflow to adhoc add file to given lists

***** maintenance
****** garbage collecting
pastebin-style timeouts
- clearweb: delete FILE from webserver
- ipfs: stop providing FILE on nodes

***** export format
  + recursive exporter (if linked file is not 'shared', then =(share-adhoc link-file)=
    - prompt "should we share FILE?" and thuis recursively for further linked files
    - don't need to be immediate, but =share-lint= should detect dangling links
    - so, could be one such maintenance tasks
- copy actions
- versioning
**** TODO init :init:
#+begin_src emacs-lisp
...
**** ol-info :module:
:PROPERTIES:
:ID:       b98ba365-6e16-46d1-a370-fb0cd5061511
:END:
:: https://stackoverflow.com/questions/10445285/org-mode-embed-links-to-info-files#10453313
:: https://orgmode.org/manual/Adding-Hyperlink-Types.html
:: https://samuelculpepper.com/share/info
:: modules/org-share/ol-info.el

a backend for =info=
#+begin_src emacs-lisp :tangle modules/org-share/ol-info.el
...
***** TODO add infofile hosting on samuelculpepper.com :@share:
relevant to [[id:eb5bf554-0804-4487-be65-8ec1fac85d4f][data sync]], [[id:edd82bc1-eb33-4170-a953-051ec2c50fae][Information and the Universe]]

this is one such "external data" as described in the above notes

***** TODO add infofile version to extract
in order of
- sha256 of build (from guix info)
- build version +  datetime
- version
- datetime
- local path (relative to infodir)
...
:: [[https://www.text2mm.com/][Text 2 MindMap]]
...
(qz/get-headline-path-insert t nil 'qz/headline-mm)
#+end_src
