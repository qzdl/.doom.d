...
(format " %s %s%s%s" (format "%s" (if (member category-icon . #2=('(#1="" nil))) #1# (concat category-icon #1# (get-text-property 0 #3='extra-space category-icon)))) (format "%s" (org-eval (vulpea-agenda-category qz/org-agenda-prefix-length))) (if (member time '(#1# nil)) #1# (format "%-12s" (concat time #1#))) (format "%s" (if (member extra . #2#) #1# (concat extra " " (get-text-property 0 #3# extra)))))
...
#+RESULTS:
| n | empty | plain | %? | :if-new | (file+head ${slug}.org #+title: ${title} |
